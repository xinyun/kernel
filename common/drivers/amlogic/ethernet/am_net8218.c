...
static ssize_t exec_write_proc(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	char buf[128];
	int ret = 0;
	...
	{
		#if (defined(CONFIG_MESON8B_TH0 )|| defined(CONFIG_MESON8B_512M_TH01))
	...
	if(strncmp(buf,"pon", 3)==0)
	{
		#ifdef CONFIG_MESON8B_TH0
		printk("____________power_state______on\n");
	...
	else if(strncmp(buf,"poff", 4)==0)
	{
		#ifdef CONFIG_MESON8B_TH0
		printk("____________power_state______off\n");
        CLEAR_AOBUS_REG_MASK(AO_GPIO_O_EN_N, 1<<15);
...
static int __init init_proc_exec(void)
{
	...
	entry = proc_create_data("power_state", 0777, NULL,&netstate_proc_fops,NULL);
	return 0;
}
