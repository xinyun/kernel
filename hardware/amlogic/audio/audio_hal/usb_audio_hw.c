...
static int start_output_stream(struct aml_stream_out *out)
{
    ...
    int i, err;
    if ((adev->card_playback < 0) || (adev->card_device < 0)) {
        return -EINVAL;
    }
	if(adev->out_device & AUDIO_DEVICE_OUT_USB_DEVICE){
		err = get_usb_cap("Playback:", &out->out_config.channels, &out->out_config.rate, adev->card_playback);
		if (err) {
	...
	out->out_pcm = pcm_open(adev->card_playback, adev->card_device, PCM_OUT, &out->out_config);
...
{
	struct aml_stream_in *in = (struct aml_stream_out *)stream;
	struct aml_audio_device *adev = in->dev;
...
static int get_usb_card(struct aml_audio_device *dev)
{
   ...
   // ALOGD("****str=%s****",str);
    ...
    adev->card_capture = card;
    ALOGD("******get_usb_card***card=%d***",adev->card_capture);
	adev->card_device= 0;
...
static int start_input_stream(struct aml_stream_in *in)
{
	...
	//get_usb_card(in);
	if ((adev->card_capture < 0) || (adev->card_device < 0)) {
...
#endif
    ALOGD("pcm_open in: card(%d), port(%d)", adev->card_capture, adev->card_device);
    in->in_pcm = pcm_open(adev->card_capture, adev->card_device, PCM_IN, &in->in_config);
                                  ...
                                  struct audio_stream_in **stream_in)
{
    ...
    ret = get_usb_cap("Capture:", &in->in_config.channels, &in->in_config.rate, adev->card_capture);
