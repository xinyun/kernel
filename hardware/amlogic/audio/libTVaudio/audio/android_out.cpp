...
static void AudioTrackCallback(int event, void* user, void *info) {
    ...
    if(GetOutputdevice() == 1){
        bytes = buffer_read(&android_out_buffer,(char *) buffer->raw, buffer->size);
        if(bytes < 0) buffer->size = 0;
    }else if(GetOutputdevice() == 2){
        bytes = buffer_read(&DDP_out_buffer,(char *) buffer->raw, buffer->size);
        if(bytes < 0) buffer->size = 0;
    }else{
...
static int AudioTrackInit(void) {
    ...
    Status = glpTracker->set(AUDIO_STREAM_MUSIC, 0, AUDIO_FORMAT_PCM_16_BIT,
