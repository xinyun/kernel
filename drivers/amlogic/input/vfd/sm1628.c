...
void MDrv_TM1623_Clear6964RAM(void)
{
	...
	PT6964_STB_L;
	for(i=0;i<=9;i++)			                     //22 bytes
	{
		MDrv_TM1623_WriteData(0xc0|i);			//Command 2, RAM address = 0
...
void MDrv_TM1623_StandBy(void)
{
	...
	MDrv_TM1623_Write_Adr_Data(0x07, 0x00);
#if 1
	MDrv_TM1623_Write_Adr_Data(0x08, 0x00|BIT_A);
#endif
}
...
void MDrv_TM1623_LED_DISP(char U8_Ledbit, char U8_LowByte, char U8_HighByte)
{
	...
	MDrv_TM1623_Write_Adr_Data(0x00, 0x5f);
	MDrv_TM1623_Write_Adr_Data(0x02, 0x3b);
	MDrv_TM1623_Write_Adr_Data(0x04, 0x50);
	MDrv_TM1623_Write_Adr_Data(0x06, 0x3f);
...
//-------------------------------------------------------------------
bool MDrv_FP_LED_CharSet(unsigned char ledBit, unsigned char U8LEDChar)
{
	unsigned char i;
	...
	for (i = 0; i < sizeof(_char2SegmentTable) / sizeof (Char2Segment); i++)
	{
		if (U8LEDChar == _char2SegmentTable[i].u8Char) {
			...
			low = (unsigned char)_char2SegmentTable[i].u8SegmentLowByte;
			high = (unsigned char)_char2SegmentTable[i].u8SegmentHighByte;
			//for(i = 0; i < 8; i++) {
			//	if((low >> i)&0x01)
			//		dig_value[i] |= 1 << bit2seg[ledBit];
			//}
			dig_value[ledBit] = low;
...
#if 1// def D_CTL_PANEL_LED_LIGHT
extern int greenLedFlag;
void FP_Singal_LED_OnOff(bool bLightOn)  // PANEL CTL LED ON/OFF
{
    if(bLightOn)
    {
		printk("spink ##### Polling_DVBT_Singal LED ON###\n");
		#if 1
		//MDrv_TM1623_Write_Adr_Data(8, 0x0c);
		#else
		MDrv_TM1623_Write_Adr_Data(13, 0x02);
		#endif
    }
    else
    {
		printk("##### Polling_DVBT_Singal LED OFF###\n");
		#if 1
		//MDrv_TM1623_Write_Adr_Data(8, 0x08);
		#else
...
void MDrv_FrontPnl_Update(char *U8str, int colon)
{
	...
	if(colon){
		dig_value[1] |=  BIT_P;
		/*
		if(colon&0x01)	dig_value[1] |=  BIT_P;
		if(colon&0x02) 	{
...
#if 1
	MDrv_TM1623_Write_Adr_Data(8, 0x02);
#else
	MDrv_TM1623_Write_Adr_Data(13, 0x00); // power on led
#endif
